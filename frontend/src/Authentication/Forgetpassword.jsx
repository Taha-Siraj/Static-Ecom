...
const ForgetPassword = () => {
    ...
    const handleSendCode = async (e) => {
        ...
        try {
            setLoader(true);
            let res = await axios.post('http://localhost:5004/api/v1/forget-password', { email });
            if (res.status === 200) {
                setLoader(false);
            ...
            <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
                ...
                {step === 1 && (
                    <div>
                        ...
                        <form onSubmit={handleSendCode} className="mt-8 space-y-6">
                            ...
                            <button
                                type="submit"
                                className={loader ? "w-full px-4 py-2 font-semibold text-white bg-indigo-600 rounded-md opacity-50 cursor-not-allowed" : "w-full px-4 py-2 font-semibold text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-300"}
                            >
                ...
                {step === 2 && (
                    <div>
                        ...
                        <p className="mt-2 text-center text-gray-600">We've sent Link 6-digit verification code to <b>{email}</b></p>
                ...
                {step === 4 && (
                    <div className="text-center">
                        ...
                        <div className="mt-6">
                            <Link to="/login" className="w-full inline-block px-4 py-2 font-semibold text-white bg-indigo-600 rounded-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-300">
                                Go to Login
                            </Link>
                        </div>
                    </div>
